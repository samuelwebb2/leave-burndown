...
def make_data(entries=(), flexed=(), **settings):
...
def entry(start, end, **extra):
    return {"label": "x", "start": start, "end": end, **extra}


def test_weekends_and_bank_holidays_dont_use_leave():
...
def test_half_day_on_the_first_day():
...
def test_half_days_on_both_ends():
...
def test_half_day_on_the_last_day_only():
...
def test_half_day_on_a_non_working_end_changes_nothing():
...
def test_flexing_a_bank_holiday_adds_a_day_and_makes_it_use_leave():
